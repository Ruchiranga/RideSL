...
$(document).ready(function() {
    ...
    $('#pencil1').click(function() {
        ...
        $('#edit_name').replaceWith(function() {
            ...
            var string = "<form onsubmit='return checkName()' id='edit_name_form' name = 'edit_name' method = post action = 'http://localhost/Ridesl/vehicleRegister/editName' >";
    ...
    $('#pencil3').click(function() {
        ...
        $('#edit_email').replaceWith(function() {
            var string = '<form onsubmit="return checkEmail()" id="edit_email_form" name = "edit_email" method = post action = "http://localhost/Ridesl/vehicleRegister/editEmail" >';
    ...
    $('#add_phone').click(function() {
        if (add_phone_no_count === 0) {
            ...
            var string = '<form  onsubmit="return checkPhoneNo()" id="edit_phone_no_form" name = "add_phone_no" method = post action = "http://localhost/Ridesl/vehicleRegister/addPhoneNo" >';
...
function changePhone($id) {
    ...
    $('#text_phone'+$id).replaceWith(function() {
        ...
        var string = '<form  onsubmit="return checkUpdatePhoneNo('+$id+')" id="edit_phone_no_form'+$id+'" name = "add_phone_no" method = post action = "http://localhost/Ridesl/vehicleRegister/updatePhoneNo/'+data+'" >';
...
function updatePhoneNoCancel($id, $index) {
    ...
    $('#edit_phone_no_form'+$id).replaceWith(function() {
        ...
        string += '<a href="http://localhost/Ridesl/vehicleRegister/dltPhoneNo/'+dltNo+'"><img onclick="return confirm(\'Are you sure you want to delete phone no '+dltNo+'?\');" id = "dlt_phone'+$id+'" src="http://localhost/Ridesl/public/images/dlt.png" alt="" style="height: 13px; width: 13px; cursor:pointer"/></a>';
        string += '</div>';
        return string;
    });
}
